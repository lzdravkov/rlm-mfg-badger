...
# === NEW: Query with automatic pagination handling ===
def query_salesforce(soql, access_token, instance_url, api_version):
    """
    Execute a SOQL query and handle pagination automatically.
    Returns all records as a list.
    """
    query_url = f"{instance_url}/services/data/v{api_version}/query"
    headers_local = {"Authorization": f"Bearer {access_token}"}
    all_records = []
    
    resp = requests.get(query_url, headers=headers_local, params={"q": soql})
    ...
    # Handle case where response might be a list (shouldn't happen, but defensive)
    if isinstance(response_data, list):
        print("⚠️ Unexpected list response from Salesforce API")
        return response_data
    
    # Normal case: response is a dict with 'records' key
    all_records.extend(response_data.get("records", []))
    
    # Handle pagination using nextRecordsUrl
    while response_data.get("nextRecordsUrl"):
        next_url = f"{instance_url}{response_data['nextRecordsUrl']}"
        resp = requests.get(next_url, headers=headers_local)
        
        if resp.status_code != 200:
            print(f"⚠️ Pagination request failed: {resp.status_code}")
            break
...
def upsert_expression_set(record, access_token, instance_url, api_version):
    ...
    records = query_salesforce(soql, access_token, instance_url, api_version)
...
def upsert_esdcd(record, access_token, instance_url, api_version):
    ...
    # Query for existence
    soql = f"SELECT Id FROM {obj_name} WHERE ExpressionSetDefinitionId = '{esd_id}'"
    found = query_salesforce(soql, access_token, instance_url, api_version)
...
def main():
    ...
    records = query_salesforce(q, access_token, instance_url, api_version)

    if not records:
        print(f"❌ Could not find ExpressionSetDefinitionVersion for {devname}")
        return
    esdv_id = records[0]["Id"]
    ...
    # === FIXED: Query target org for Product2 with pagination support ===
    uk_to_targetId_prod = {}
    if product_names:
        # Split into batches if needed (Salesforce has query length limits)
        ...
        for i in range(0, len(product_list), batch_size):
            ...
            prod_filter = ",".join(f"'{n.replace(chr(39), chr(92) + chr(39))}'" for n in batch)  # Escape single quotes
            q1 = f"SELECT Id, Name FROM Product2 WHERE Name IN ({prod_filter})"
            
            print(f"📊 Querying Product2 batch {i//batch_size + 1} ({len(batch)} items)...")
            records = query_salesforce(q1, access_token, instance_url, api_version)
            
            for r in records:
                uk_to_targetId_prod[r["Name"]] = r["Id"]
        
        print(f"✅ Retrieved {len(uk_to_targetId_prod)} Product2 records")

    # === FIXED: Query target org for ProductClassification with pagination support ===
    uk_to_targetId_class = {}
    if classification_names:
        class_list = list(classification_names)
        batch_size = 100
        
        for i in range(0, len(class_list), batch_size):
            batch = class_list[i:i + batch_size]
            class_filter = ",".join(f"'{n.replace(chr(39), chr(92) + chr(39))}'" for n in batch)
            q2 = f"SELECT Id, Name FROM ProductClassification WHERE Name IN ({class_filter})"
            
            print(f"📊 Querying ProductClassification batch {i//batch_size + 1} ({len(batch)} items)...")
            records = query_salesforce(q2, access_token, instance_url, api_version)
            
            for r in records:
                uk_to_targetId_class[r["Name"]] = r["Id"]
        
        print(f"✅ Retrieved {len(uk_to_targetId_class)} ProductClassification records")

    # === FIXED: Query target org for ProductRelatedComponent with pagination support ===
    uk_to_targetId_prc = {}
    if prc_parent_names:
        prc_list = list(prc_parent_names)
        batch_size = 100
        
        for i in range(0, len(prc_list), batch_size):
            batch = prc_list[i:i + batch_size]
            prc_filter = ",".join(f"'{n.replace(chr(39), chr(92) + chr(39))}'" for n in batch)
            q3 = f"""
            SELECT Id,
                ParentProduct.Name,
                ChildProduct.Name,
            ...
            print(f"📊 Querying ProductRelatedComponent batch {i//batch_size + 1} ({len(batch)} items)...")
            records = query_salesforce(q3, access_token, instance_url, api_version)
            ...
            for r in records:
                if r.get("ParentProduct"):
                    ...
                    uk_to_targetId_prc[uk] = r["Id"]
        
        print(f"✅ Retrieved {len(uk_to_targetId_prc)} ProductRelatedComponent records")
    ...
    existing_esc_records = query_salesforce(esc_query, access_token, instance_url, api_version)
    existing_esc_ids = [r["Id"] for r in existing_esc_records]
